...
public class PlayerControllerComponent : ActorComponent
{
    ...
    private readonly ConfigStatsPlayer m_configStats;
    
    public PlayerControllerComponent(Actor owner,Rigidbody rb, ConfigStatsPlayer configStats) : base(owner)
    {
        m_rb = rb;
    ...
    {
        if (m_configStats == null) return;

        Vector3 origin = m_rb.position + Vector3.up * m_configStats.GroundCheckHeight;
        bool grounded = Physics.Raycast(origin, Vector3.down, m_configStats.GroundCheckDistance, m_configStats.GroundLayer);

        Gizmos.color = grounded ? Color.green : Color.red;
        Gizmos.DrawLine(origin, origin + Vector3.down * m_configStats.GroundCheckDistance);
